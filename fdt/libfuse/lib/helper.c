...
#include "wizard_tests.c"
#include "testsuite_run.c"
			       ...
			       int compat)
{
	...
	// Initialise differently depending on the FDT tool in use
	char * tool_ident = getenv("FDT_TOOL");

	if(strcmp(tool_ident, "wizard") == 0) {
		/* Wizard - run tests and prevent the mount */
		printf("[libfuse] FDT_TOOL is 'wizard'\n");
		fwizard_init(op, op_size);
		goto err_unmount;
	} else if(strcmp(tool_ident, "testsuite") == 0) {
		/* Test Suite */
		printf("[libfuse] FDT_TOOL is 'testsuite'\n");
		testsuite_init(op, op_size, *mountpoint);
		goto err_unmount;
	} else if(strcmp(tool_ident, "debugger") == 0) {
		/* Debugger - so continue with normal mount */
		printf("[libfuse] FDT_TOOL is 'debugger'\n");
		return fuse;
	} else {
		/* Unknown tool.. so allow a normal mount */
		printf("[libfuse] FDT_TOOL is '%s' (unknown tool)\n", tool_ident);
		return fuse;
	}
	err_unmount:
		fuse_unmount_common(*mountpoint, ch);
